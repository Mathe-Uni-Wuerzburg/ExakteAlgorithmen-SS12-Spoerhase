...
\begin{section}{Dominating Set}
  ...
  \begin{proof}
    ...
    %Wir definieren für Teilmengen \(D \subseteq V\) die Menge  und 
    %\[\gamma \isDefinedBy \min \{ |D| : D \text{ dominiert } G \}.\] 
    %Dann ist die Menge  in \(D\) enthalten, denn ein \(v \in I \setminus N(J \cap D)\) liegt in \(I\), jedoch nicht in der Nachbarschaft von \(J \cap D\). Damit wird \(v\) nicht von \(J \cap D\) dominiert. Da aber \(D\) eine dominierende Menge ist, muss \(v\) von \(I \cap D\) dominiert werden. .... \(v\) liegt nicht in \(N(J \cap D)\) und nicht in \(N(I \cap D)\), also \(v \notin N(D)\). Da \(v \notin N(D)\) und \(D\) dominierende Menge, gilt also \(v \in D\).
  \end{proof}  
